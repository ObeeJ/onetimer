...
export default function SurveyCard({ survey }: { survey: Survey }) {
  ...
  const hasValidId = survey.id && survey.id !== 'undefined' && survey.id !== 'null'
  ...
  return (
    <Card className="group h-full min-w-0 w-full overflow-hidden rounded-2xl border border-slate-200/60 bg-white/80 backdrop-blur-xl shadow-sm transition-all duration-300 hover:shadow-lg hover:shadow-slate-200/50 hover:-translate-y-1">
      ...
      <CardContent className="flex-1 space-y-4 pb-4">
        <p className="line-clamp-3 text-sm font-medium leading-relaxed text-slate-600 break-words">{survey?.description || "No description available"}</p>
      ...
      <CardFooter className="flex items-center gap-3 pt-4">
        <Button
          ...
          disabled={!hasValidId}
        >
          <Link href={hasValidId ? `/filler/surveys/${survey.id}` : "#"} className="truncate">Details</Link>
        </Button>
        <Button
          asChild
          ...
          size="lg"
          disabled={!eligible || !hasValidId}
        >
          <Link href={eligible && hasValidId ? `/filler/surveys/${survey.id}/take` : "#"} className="truncate">
            {!hasValidId ? "Invalid Survey" : eligible ? "Start Survey" : "Not Eligible"}
          </Link>
        </Button>
      </CardFooter>
    </Card>
